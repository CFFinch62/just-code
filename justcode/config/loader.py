# Just Code Editor - Config Loader
...
class ConfigLoader:
    ...
    def _get_config_dir(self) -> Path:
        ...
        # Use XDG standard: ~/.config/justcode/
        config_home = os.environ.get('XDG_CONFIG_HOME')
        if config_home:
            return Path(config_home) / 'justcode'
        return Path.home() / '.config' / 'justcode'
    ...
    def _get_default_config_dir(self) -> Path:
        ...
        if getattr(sys, 'frozen', False):
            ...
            return base_path / 'justcode' / 'resources' / 'default_configs'
        else:
    ...
    def get_icon_path(self) -> Optional[Path]:
        ...
        if sys.platform == 'win32':
            icon_name = 'justcode.ico'
        else:
            icon_name = 'justcode.png'

        # Get the resources directory (handles both dev and bundled)
        if getattr(sys, 'frozen', False):
            base_path = Path(sys._MEIPASS)
            icons_dir = base_path / 'justcode' / 'resources' / 'icons'
        else:
